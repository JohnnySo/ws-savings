...
class ExcelFileReaderApplicationService: FileReaderApplicationService {
    ...
    private fun buildMovement(row: Row, order: Int, fieldPositions: Map<Field, Int>): PreMovement{
        val operationDate = fieldPositions[Field.OPERATION_DATE]?.let { row.getCell(it).localDateTimeCellValue.toLocalDate() }
        ...
        if (operationDate == null || description == null || amount == null) {
            throw FileParseException("The file format it is not valid. " +
            ...
            return PreMovement(operationDate, description, Money.of(amount), Order(order),
